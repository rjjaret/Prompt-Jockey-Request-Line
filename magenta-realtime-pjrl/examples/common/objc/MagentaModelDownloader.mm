...
static NSString *const kMagentaRTModelFolderBookmarkKey = @"MagentaRT_ModelFolderBookmark";
...
+ (void)downloadFiles:(NSArray<NSDictionary *> *)files
    ...
    NSURLSessionDownloadTask *downloadTask = [[NSURLSession sharedSession] downloadTaskWithRequest:request completionHandler:^(NSURL *location, NSURLResponse *response, NSError *netError) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:localDestPath isDirectory:&existingIsDir]) {
            if (existingIsDir) {
                // Some remote listings can include directory-like entries; if the
                // destination is already a directory, skip this item and continue.
...
+ (BOOL)areSharedResourcesValid {
    ...
    NSString *customPath = [defaults objectForKey:@"MagentaRT_CustomResourcesPath"];

    NSData *bookmark = [defaults objectForKey:kMagentaRTModelFolderBookmarkKey];
    NSURL *modelsDir = nil;
    ...
    NSMutableArray<NSString *> *candidateResourceDirs = [NSMutableArray array];
    if (customPath.length > 0) {
        [candidateResourceDirs addObject:customPath];
    }

    if (modelsDir.path.length > 0) {
        NSString *siblingResources = [modelsDir.path stringByAppendingPathComponent:@"resources"];
        [candidateResourceDirs addObject:siblingResources];
    }

    [candidateResourceDirs addObject:[NSString stringWithUTF8String:magentart::paths::get_resources_dir().c_str()]];

    BOOL valid = NO;
    for (NSString *resourcesDir in candidateResourceDirs) {
        ...
        if ([self hasFileMatchingPattern:cocaPattern] && [self hasFileMatchingPattern:streamPattern]) {
            valid = YES;
            break;
        }
    }
