    ...
    >
      ...
      <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none' }}>
        {prompts.map((p) => {
          ...
          const displayLabel = p.isSmsRequest ? "'" + p.label + "'" : p.label;
          const inputValue = p.isAudio
            ? `♪ ${displayLabel}`
            : (editingPromptId === p.id ? p.label : displayLabel);
          const showPhoneIcon = !!p.isSmsRequest;
          return (
            <div
              key={p.id}
              className={`prompt-label${showPhoneIcon ? ' sms-request-label' : ''}`}
              style={{
                position: 'absolute',
                left: p.x,
            ...
            >
              {showPhoneIcon && (
                <span className="sms-request-phone" aria-hidden="true">📱</span>
              )}
              <input
                ...
                onBlur={() => {
                  if (p.isAudio) return;
                  setEditingPromptId((current) => (current === p.id ? null : current));
                  if (!p.label.trim()) {
